...
in
{
  options.nixVegas.desktop = {
    enable = lib.mkEnableOption "apply the desktop hardware groups + pkcs11 (pcsc) access to all normal users";
    groups = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ ];
      description = "extra groups every normal user gets in desktop mode (set by desktop.nix)";
    };
  };
  ...
  config = {
          ...
          (
            ...
            // lib.optionalAttrs (lib.elem "wheel" (u.extraGroups or [ ])) {
              extraGroups = lib.unique (
                u.extraGroups
                ++ adminExtraGroups
                # In desktop mode every user also gets the desktop hardware groups
                # (list carried by desktop.nix via nixVegas.desktop.groups).
                ++ lib.optionals config.nixVegas.desktop.enable config.nixVegas.desktop.groups
              );
            }
          )
          {
    ...
    # In desktop mode, pkcs11 smartcard (pcsc) access for ALL normal users, not
    # just numinit. Reading config.users.users to SET a different option
    # (pcsc.users) is safe -- no self-reference like setting users.users would be.
    security.pkcs11.pcsc.users = lib.mkIf config.nixVegas.desktop.enable (
      builtins.attrNames (lib.filterAttrs (_: u: u.isNormalUser or false) config.users.users)
    );
  };
}
